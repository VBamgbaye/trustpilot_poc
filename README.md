...
1. **Ingest data**
   ...
   - Executes a lightweight Great Expectations suite that mirrors the same rules (required columns, allowed ratings, parsable dates, email/IP formats, unique review IDs) and reports a pass/fail summary per file.
...
## Extending the PoC
- Implement the `app.retention` module referenced by `make retention` to enforce the declared retention windows.
- Broaden pytest coverage to include retention workflows, catalog endpoints, and negative API scenarios.
- Consider containerizing the app and wiring CI/CD tasks that invoke `make lint`, `make test`, and `make backup`.
- Harden auth (rotate tokens, integrate with real IDP) and consider rate limiting before productionizing.

## Support & Troubleshooting
- If parquet support fails during ingestion, a warning indicates the CSV fallback path; install `pyarrow` or `fastparquet` to restore Parquet outputs.
- Audit logs default to stdout; set `LOG_FILE` in `.env` to capture them for long-running services.
- Review `.env` and `config/catalog.yml` together to ensure governance metadata stays synchronized with code changes.
